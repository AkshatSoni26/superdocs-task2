...
logger = logging.getLogger("superdocs_service")
...
class SuperDocsClientService:
    ...
    async def upload_document(self, filename: str, content: bytes | str) -> SuperDocsUploadResponse:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            data = response.json()
            return SuperDocsUploadResponse(**data)
    ...
    async def send_chat_instruction(self, request: SuperDocsChatRequest) -> SuperDocsChatResponse:
        ...
        if self.mock_mode:
            # Generate realistic SuperDocs proposed changes formatted as double-encoded JSON string
            mock_diffs = [
                {
            ...
            # SuperDocs Gotcha: raw proposed changes arrive as a JSON-encoded string requiring a second parse
    ...
    async def export_document(self, document_id: str, format_type: str = "pdf") -> SuperDocsExportResponse:
        ...
        if self.mock_mode:
            return SuperDocsExportResponse(
                ...
                download_url=f"/api/v1/superdocs/download/{document_id}.{format_type}",
